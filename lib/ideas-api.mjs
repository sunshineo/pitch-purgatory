...
function validatePublishPayload(body) {
  ...
  return {
    ...
    parentIdeaId: cleanString(body.parentIdeaId, 80) || null,
    source: cleanString(body.source, 24) || 'original'
  };
}
...
export async function handleListIdeas(req, res) {
  try {
    ...
    const sort = ['recent', 'blessed', 'damned', 'controversial'].includes(requestedSort)
      ? requestedSort
      : 'recent';
...
export async function handleVoteIdea(req, res, idOrSlug) {
  ...
  try {
    ...
    const idea = await voteOnIdea({ idOrSlug: key, visitorId, voteType });
    if (!idea) {
      res.status(404).json({ error: 'Idea not found.' });
      return;
    }
...
export async function handleCreateComment(req, res, idOrSlug) {
  ...
  try {
    const comment = await createComment({
      idOrSlug: key,
      authorDisplayName: cleanString(body.authorDisplayName, 64) || 'Anonymous heckler',
      body: commentBody,
      stance
    });
...
export async function handleListForks(req, res, idOrSlug) {
  const key = cleanString(idOrSlug, 120);
  ...
  try {
    const forks = await listForks(key);
    res.status(200).json({ forks });
  } catch (error) {
    res.status(500).json({ error: error.message || 'Could not load forks.' });
  }
}
