  ...
  const result = await getPool().query(
    `
      ...
      RETURNING *,
        ...
        (SELECT image FROM users WHERE id = $13) AS author_image
    `,
    [
      id,
      slug,
...
export async function listIdeas({ limit = 24, offset = 0, sort = 'recent' } = {}) {
  await ensureSchema();
...
export async function createComment({
  ...
  const result = await getPool().query(
    `
      ...
      SELECT inserted.*, owner.image AS author_image
      FROM inserted
